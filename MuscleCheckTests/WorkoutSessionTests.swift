...
struct WorkoutSessionTests {
    ...
    func testDecodesLegacyJSONWithoutSetsAndReps() throws {
        ...
        #expect(session.distanceMeters == nil)
    }
    ...
    @Test
    func testDecodesPreMetricJSONWithoutDurationAndDistance() throws {
        // Shape encoded before duration/distance existed (2.1.x with sets/reps).
        let json = """
        {
        ...
        #expect(session.weight == 60)
        #expect(session.sets == 4)
        #expect(session.durationSeconds == nil)
        #expect(session.distanceMeters == nil)
    }
    ...
    func testRoundTripPreservesAllFields() throws {
        let original = WorkoutSession(weight: 90, sets: 5, reps: 8, durationSeconds: 1800, distanceMeters: 5200, date: Date())
        ...
        #expect(decoded.distanceMeters == 5200)
